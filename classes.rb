...
# enemy squirtle, thundershock
  def choose_turn(your_pokemon, enemy_pokemon)
    puts "#{your_pokemon.name}'s starting health is #{your_pokemon.health}. His type is #{your_pokemon.type.type_name}."
    puts "#{enemy_pokemon.name}'s starting health is #{enemy_pokemon.health}. Her type is #{enemy_pokemon.type.type_name}."

    while enemy_pokemon.health > 0 do
      puts "Which move would you like to do?"
      ...
      #puts "#{enemy_pokemon.name}'s health is now #{enemy_pokemon.health}"
    end
    puts "#{enemy_pokemon.name} has fainted!"
  end

  def take_turn(your_pokemon, enemy_pokemon, move_used)
    system 'clear'
    puts "#{your_pokemon.name} used #{move_used.move_name}!"
    enemy_type = enemy_pokemon.type #water - the type of the opposing pokemon
    damage_dealt = move_used.move_damage #the damage to be dealt to pokemon
  ...
  def take_damage(pokemon, pokemon_type, original_damage_dealt, move_type) #move_type should be the string of the spell
    ...
    puts "original damage dealt is #{original_damage_dealt}, damage modifier is #{damage_modifier}, thus full damage dealt is #{full_damage_dealt}"
  end

  def update_health(pokemon, damage)
      pokemon.health -= damage
      puts "You dealt #{damage}"
      if pokemon.health > 74
        puts "#{pokemon.name}'s health is now #{Paint[pokemon.health, :green]}"
      elsif pokemon.health > 24
        puts "#{pokemon.name}'s health is now #{Paint[pokemon.health, :yellow]}"
      else
        puts "#{pokemon.name}'s health is now #{Paint[pokemon.health, :red]}"
      end
      #puts "original damage dealt is #{original_damage_dealt}, damage modifier is #{damage_modifier}, thus full damage dealt is #{full_damage_dealt}"
  end
...
class Move
  def initialize(move_name, move_damage, move_type)
    @move_name = move_name
    @move_damage = move_damage
    @move_type = move_type
  end
  attr_accessor :move_name, :move_damage, :move_type
end
